...
# This file is vendored from NPZ.jl
# I removed dependence on a zlib package that had poor compilation/loading
# performance. Instead, we use CodecZlib for decompression.
# (GJL 13 Dec 2024)
...
function npzread(filename::AbstractString, vars...)
    ...
    if samestart(b, ZIPMagic)
        error("Zipped npz numpy files not supported")
...
function npzwritearray(
    ...
    dict = "{'descr': '$descr', 'fortran_order': $forder, 'shape': $(Tuple(shape)), }"
...
function npzwritearray(f::IO, x::AbstractArray; fortran_order=true)
    return npzwritearray(f, reinterpret(UInt8, vec(x)), eltype(x), size(x); fortran_order)
end

function npzwritearray(f::IO, x::Number; fortran_order=true)
    return npzwritearray(f, reinterpret(UInt8, [x]), typeof(x), (); fortran_order)
end
